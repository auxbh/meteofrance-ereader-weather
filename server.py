...
def get_weather_data_from_request(req):
    ...
    logger.debug(
        "Weather data from %s @ %s",
        data["now"]["api_provider"],
        data["now"]["time"],
    )
    info = {}
    report_time = time.strptime(data["now"]["time"], "%Y-%m-%d %H:%M:%S")
    timestamp = WeatherUtils.get_localtime()
    info["day"] = WeatherUtils.get_french_day_str(timestamp)
...
@app.route("/ereader_image", strict_slashes=False)
def ereader_image():
    try:
        import io
        ...
        data["now"]["icon_data"] = get_base64_icon(
            data["now"]["icon"], data["info"]["icon_ext"], static_dir
        )
        for item in data["hourly"]:
            item["icon_data"] = get_base64_icon(
                item["icon"], data["info"]["icon_ext"], static_dir
            )
        ...
        output = io.BytesIO()
        logger.debug("Saving PNG to output stream...")
        image.save(output, format="PNG")
        output.seek(0)
        logger.debug("Returning PNG response...")
